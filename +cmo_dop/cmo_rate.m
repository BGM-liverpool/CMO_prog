function rate = cmo_rate(y, param, dopuse, nulim, mort, APA, remtype)
%
...
rate.theta = rate.tch.*rate.fC; % cellular Chl:C ratio, gchl/mol      
rate.SI = 1-exp(-rate.tch.*rate.aim);%light saturation, dimensionless
else                       
...
rate.VNh0 = rate.an./(sqrt(rate.an./rate.VNmax) + 1.0).^2;% Local potential N aquisition rate, mol/mol/d
rate.VPh0 = rate.ap./(sqrt(rate.ap./param.V0P) + 1.0).^2;% Local potential P aquisition rate, mol/mol/d
...
if strcmp(remtype,'MM') % enzyme Menchaelies-Menton
 rate.Vapa_max = param.Vapa0.*rate.fDOP;% umolP/L/d
 rate.remdop = rate.Vapa_max.*DOP./(DOP+param.km);% umolP/L/d
elseif strcmp(remtype,'linear') % enzyme Menchaelies-Menton
 rate.remdop = param.lamda_dop.*rate.fDOP.*DOP; %umolP/L/d    
end
end

rate.VC0= rate.fC.*rate.A - param.zN.*rate.VN-param.Rm; % gross C aquisition rate, mol/mol/d
